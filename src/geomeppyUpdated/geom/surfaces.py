...
def get_adjacencies(surfaces):
    ...
    test_poly = Polygon3D(test_surface.coords)
    logger.debug(f"{test_poly=}")
    logger.debug(f"{test_poly.as_shapely()}")

    adjacencies = defaultdict(list)  # type: defaultdict
    # find all adjacent surfaces
    for s1, s2 in combinations(surfaces, 2):
        with logger.contextualize(surface_pair=f"({s1.Name}, {s2.Name})"):
...
def populate_adjacencies(adjacencies, s1, s2):
    ...
    if not almostequal(abs(poly1.distance), abs(poly2.distance), 4):
        # logger.debug("Polygons are NOT adjacent")
        return adjacencies
    ...
    sp1 = poly1.as_shapely()
    sp2 = poly1.as_shapely()
    sp_intersect = sp1.intersects(sp2)
    ints = shapely.intersection(sp1, sp2)
    if ints.area > 0:
        logger.debug(f"SHAPELY SAYS INTERSECTS: {ints.area}")
    else:
        dist = sp1.distance(sp2)
        logger.debug(f"DIST={dist}: {sp1}, {sp2}")

    if intersection:
        logger.debug("Polygons are intersecting..")
